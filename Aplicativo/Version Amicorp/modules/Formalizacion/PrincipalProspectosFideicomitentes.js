...
//No permitir editar el campo informativo de la p�gina principal:
...
function operacionExitosa() {
  alert("La operaci�n se realiz� correctamente");
  //cargaPrincipalProspectosFideicomitentes();
  ocultaObjs("cmdAceptar"); //Mostrar el bot�n Aceptar y Cancelar
  SA(GI("cmdCancelar"), "value", "Regresar");//Colocar la leyenda Regresar en vez de Cancelar al bot�n
...
function despliegaPantallaMantenimientoProspectosFideicomitentes(obj, result) {
  ...
  //No permitir editar el campo informativo de la p�gina de mantenimiento:
  //1. Nombre del proyecto (Nombre del negocio)
...
function loadCatalogo() {
  ...
  {
    muestraObjs("cmdAceptar,cmdCancelar"); //Mostrar el bot�n Aceptar y Cancelar
    formsLoaded();
  }
}
...
function catLoaded() {
  ...
  if(operacion==MODIFICAR)//Si se trata de una modificaci�n, no permitir modificar la PK
  {
    muestraObjs("cmdAceptar"); //Mostrar el bot�n Aceptar y Cancelar
  ...
  muestraObjs("cmdCancelar"); //Mostrar el bot�n Regresar
...
function AltaOModificaInfo() {
  ...
  else if(operacion==MODIFICAR && fvMantenimientoFideicomitentes.checkForm())//Se trata de una modificaci�n
  {
    showWaitLayer();
...
function mostrarDatosInformativos(parametroPantalla) {
  ...
  {
    var Secuencial = JSON.parse("{\"id\":\"numSecAfidben\",\"Anteproyecto\":0,\"Fideicomisarios\":\"FIDEICOMITENTE\"}");
