...
tablaGarantiasData[5] = "disponibleBien,100px";
tablaGarantiasData[6] = "pagosBien,100px";
tablaGarantiasData[7] = "saldoBien,100px";
//tablaGarantiasData[5] = "fgarFecInicio,100px";
tablaGarantiasData[8] = "fgarCveStatus,100px";
...
tablaBienesGarData[6] = "disponibleBien,110px";
tablaBienesGarData[7] = "pagosBien,110px";
tablaBienesGarData[8] = "saldoBien,110px";
tablaBienesGarData[9] = "forsCveStatus,110px";


var tablaBienesGarPagosData = new Array();
tablaBienesGarPagosData[0] = "fpbIdFideicomiso,100px";
tablaBienesGarPagosData[1] = "fpbIdSubcuenta,100px";
tablaBienesGarPagosData[2] = "fpbIdGarantia,100px";
tablaBienesGarPagosData[3] = "fpbIdBienGarantia,100px";
tablaBienesGarPagosData[4] = "fpbIdCobro,100px";
tablaBienesGarPagosData[5] = "fpbImporteFormatted,100px";
tablaBienesGarPagosData[6] = "fpbTxtMoneda,100px";
tablaBienesGarPagosData[7] = "fpbTipoCambioFormatted,100px";
tablaBienesGarPagosData[8] = "fpbImporteExtFormatted,100px";
tablaBienesGarPagosData[9] = "fpbFecha,100px";
...
function cargaMantenimientoGarantias(Modo){
    ...
    if((isDefinedAndNotNull(pkInfo) || modo == OPER_ALTA)){//
        var urlCliente = "modules/Garantias/DerechosCobro/MantenimientoDerechosCobro.do";
...
function cargaPrincipalGarantias() {
  onButtonClickPestania("Garantias.DerechosCobro.PrincipalDerechosCobro","");
  hideWaitLayer();
}
...
function consultaBienesGarPagos()
{
  GI('cmdAceptarPago').click();
}
...
function ejecutaFuncion(){
    ...
    objCargaBienesGarParam.Importe=eval(GI("forsImpBien").value);  
    ...
    objCargaBienesGarParam.ImpUltValua=eval(GI("forsImpUltValua").value);  
...
function cargaMantenimientoBienesGarPagos(Modo){
    modo = Modo;
    if((isDefinedAndNotNull(pkInfoBienesGarPagos)&&modo==OP_CONSULTAR)||modo==OP_REGISTRO)
    {
        var urlCliente = "modules/Garantias/BienesGarPagos/MantenimientoBienesGarPagos.do";
        makeAjaxRequest(urlCliente, "HTML", despliegaPantallaMantenimientoBienesGarPagos, null);
    }
    else
    {
...
function despliegaPantallaMantenimientoBienesGarPagos(obj, result) 
{
  GI("dvPantalla").innerHTML = result;
  initForms();
  fvcatfBienesgarPagos.setup({
    formName      : "frmMantenimientoBienesGarPagos",
    tipoAlert     : 1,
    alertFunction : BaloonAlert,
    sendObjToAlert: true
  });
  ...
  Calendar.setup({
    inputField     :    "fpbFecha",   // id of the input field
    button         :    "fpbFecha",
    ifFormat       :    "%d/%m/%Y", //"%a %e/%b/%Y",       // format of the input field
    showsTime      :    false,
    timeFormat     :    "24",
...
function loadCatalogoBienesGarPagos()
{
  deshabilitaPK(arrIdPKBienesGarPagos);// BLOQUEAR LOS CAMPOS DE LLAVE PRIMARIA PERTENECIENTES A LA TABLA F__PAGOS_BIENES
       
  if(modo == OP_CONSULTAR) 
  {
    deshabilitaObjetos(GI("frmMantenimientoBienesGarPagos"));
    
    GI("fpbIdFideicomiso").value = pkInfoBienesGarPagos.fpbIdFideicomiso;
    GI("fpbIdSubcuenta").value = pkInfoBienesGarPagos.fpbIdSubcuenta;
    GI("fpbIdGarantia").value = pkInfoBienesGarPagos.fpbIdGarantia;
    GI("fpbIdBienGarantia").value = pkInfoBienesGarPagos.fpbIdBienGarantia;
    GI("fpbIdCobro").value = pkInfoBienesGarPagos.fpbIdCobro;
    GI("fpbImporte").value = pkInfoBienesGarPagos.fpbImporte;
    GI("fpbCveMoneda").value = pkInfoBienesGarPagos.fpbCveMoneda;
    //GI("fpbTipoCambio").value = pkInfoBienesGarPagos.fpbTipoCambio;
    GI("fpbFecha").value = pkInfoBienesGarPagos.fpbFecha;
    GI("fpbComentario").value = pkInfoBienesGarPagos.fpbComentario;
    
  }
  else
  {
    GI("fpbIdFideicomiso").value = pkInfoBienesGar.fgrsIdFideicomiso;
    GI("fpbIdSubcuenta").value = pkInfoBienesGar.fgrsIdSubcuenta;
    GI("fpbIdGarantia").value = pkInfoBienesGar.forsCveTipoGarantia;
    GI("fpbIdBienGarantia").value = pkInfoBienesGar.forsIdGarantia;
    
  }
  
  catLoadedBienesGarPagos();
        
}

function catLoadedBienesGarPagos()
{
  if(modo==OP_REGISTRO)
  {
    muestraObj("cmdAceptarPagos");
  }
  muestraObj("cmdCancelarPagos");
  formsLoaded();
}


function ejecutaFuncionBienesGarPagos()
{
  if(fvcatfBienesgarPagos.checkForm())
  {
      var imdDisponible = replaceAll(pkInfoBienesGar.disponibleBien,",","");
  
      if(Number(imdDisponible)<Number(GI("fpbImporte").value))
      {
        alert("Importe de Cobro debe ser menor al importe disponible del Bien: "+pkInfoBienesGar.forsImpUltValuaFormatted);
        return;
      }
  
      var objCargaBienesGarParam = JSON.parse("{\"id\":\"funRegistroBienesGarPago\"}");
      objCargaBienesGarParam.numFiso=eval(GI("fpbIdFideicomiso").value);  
      objCargaBienesGarParam.numScta=eval(GI("fpbIdSubcuenta").value);
      objCargaBienesGarParam.numIdGarantia=eval(GI("fpbIdGarantia").value);
      objCargaBienesGarParam.numIdBienGarantia=eval(GI("fpbIdBienGarantia").value);
      objCargaBienesGarParam.numIdCobro=eval(GI("fpbIdCobro").value);
      objCargaBienesGarParam.importe=eval(GI("fpbImporte").value);  
      objCargaBienesGarParam.numMoneda=eval(GI("fpbCveMoneda").value);
      objCargaBienesGarParam.fecha=GI("fpbFecha").value;
      objCargaBienesGarParam.comentario=GI("fpbComentario").value;  
      objCargaBienesGarParam.operacionFuncion="ALTA"; 
      
      var url = ctxRoot + "/executeRef.do?json=" + JSON.stringify(objCargaBienesGarParam);
      //alert(url);
      makeAjaxRequest(url, "HTML", ejecutaFuncionBienesGarPagosRes, null);
  }

}


function ejecutaFuncionBienesGarPagosRes(obj,result)
{
  var res = JSON.parse(result);
  ...
  onButtonClickPestania("Garantias.DerechosCobro.PrincipalDerechosCobro","");
        break;
      case 1:
        alert("Ya existe no. de Cobro");
        break;
        
      case 2:
        alert("Se ha eliminado el Pago Correctamente");
        consultaBienesGarPagos();
        break;
      ...
      case 7:
        ...
        break;
      case 8:
        alert("Error Contable");
        break;
      case -1:
        alert("Ocurrio un error inesperado");
        break;
      default:
        alert("No existe Operación Contable : "+res.RESULTADO);
        break;
    }
...
function eliminarPagoBienes()
{
  if(isDefinedAndNotNull(pkInfoBienesGarPagos))
  {
      var objCargaBienesGarParam = JSON.parse("{\"id\":\"funRegistroBienesGarPago\"}");
      objCargaBienesGarParam.numFiso=pkInfoBienesGarPagos.fpbIdFideicomiso;  
      objCargaBienesGarParam.numScta=pkInfoBienesGarPagos.fpbIdSubcuenta;
      objCargaBienesGarParam.numIdGarantia=pkInfoBienesGarPagos.fpbIdGarantia;
      objCargaBienesGarParam.numIdBienGarantia=pkInfoBienesGarPagos.fpbIdBienGarantia;
      objCargaBienesGarParam.numIdCobro=pkInfoBienesGarPagos.fpbIdCobro;
      objCargaBienesGarParam.importe=pkInfoBienesGarPagos.fpbImporte;
      objCargaBienesGarParam.numMoneda=pkInfoBienesGarPagos.fpbCveMoneda;
      objCargaBienesGarParam.fecha=pkInfoBienesGarPagos.fpbFecha;
      objCargaBienesGarParam.comentario=pkInfoBienesGarPagos.fpbComentario;
      objCargaBienesGarParam.operacionFuncion="BAJA"; 
...
function consultaSaldoTotalBienesGar()
{
  var objSaldosBienesGar = JSON.parse("{\"id\":\"consultarBienesGarSaldoTotal\"}");
  
  objSaldosBienesGar.Fiso = pkInfo.fgarIdFideicomiso;
  objSaldosBienesGar.SubFiso = pkInfo.fgarIdSubcuenta;
  objSaldosBienesGar.CveTipoGar = pkInfo.fgarCveGarantia;
  
  var url = ctxRoot + "/getRef.do?json=" + JSON.stringify(objSaldosBienesGar);
  //alert(url);
  makeAjaxRequest(url, "HTML", consultaSaldoTotalBienesGarRes, null);
}


function consultaSaldoTotalBienesGarRes(obj,result)
{
  var res = JSON.parse(result)[0];
  
  var sdvSaldos = "SALDO TOTAL:  ";
  
  if(isDefinedAndNotNull(res))
  {
      GI("dvSaldoBien").innerHTML= sdvSaldos+res.saldoBien;
  }
  else
  {
      GI("dvSaldoBien").innerHTML=sdvSaldos+"0.00";
  }
}
